...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default=".", help="Output directory for WCSes")
    ...
    command += " --program=%s --filter=%s" % (args.field, args.filter)
    command += " --workDirRoot=%s" % (args.output if args.output is not None else ".")
    if args.doMatchPsf:
        command += " --doMatchPsf"
